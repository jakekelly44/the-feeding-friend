...
export function generateFeedingPlanPDF(data: PetData): void {
  ...
  // ===== PAGE 2: Food Recommendations (OLD STYLE with Amazon links) =====
  doc.addPage();
  ...
  categories.forEach((cat) => {
    ...
    catFoods.forEach((food) => {
      ...
      // Brand name with Amazon link if available
      if (food.amazon_url) {
        doc.setTextColor(0, 102, 204); // Blue for link
        doc.textWithLink(food.brand, margin + 5, yPos + 8, { url: food.amazon_url });
      } else {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(100, 100, 100);
      const productName = food.sku_name.substring(0, 45) + (food.sku_name.length > 45 ? '...' : '');
      
      // Product name with link if available
      if (food.amazon_url) {
        doc.setTextColor(0, 102, 204);
        doc.textWithLink(productName, margin + 5, yPos + 15, { url: food.amazon_url });
      } else {
        doc.text(productName, margin + 5, yPos + 15);
      }
